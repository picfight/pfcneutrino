...
var (
	...
	ourKnownTxsByBlock = make(map[chainhash.Hash][]*pfcutil.Tx)

	// ourKnownTxsByFilteredBlock lets the rescan goroutine keep track of
	// transactions we're interested in that are in the blockchain we're
	// following as signalled by OnFilteredBlockConnected and
	// OnFilteredBlockDisconnected.
	ourKnownTxsByFilteredBlock = make(map[chainhash.Hash][]*pfcutil.Tx)
)

// secSource is an implementation of pfcwallet/txauthor/SecretsSource that
// stores WitnessPubKeyHash addresses.
type secSource struct {
	keys    map[string]*pfcec.PrivateKey
	scripts map[string]*[]byte
	params  *chaincfg.Params
}

func (s *secSource) add(privKey *pfcec.PrivateKey) (pfcutil.Address, error) {
	pubKeyHash := pfcutil.Hash160(privKey.PubKey().SerializeCompressed())
	addr, err := pfcutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.params)
	if err != nil {
		return nil, err
	}
...
func (s *secSource) GetKey(addr pfcutil.Address) (*pfcec.PrivateKey, bool,
	error) {
...
func (s *secSource) GetScript(addr pfcutil.Address) ([]byte, error) {
...
func newSecSource(params *chaincfg.Params) *secSource {
	return &secSource{
		keys:    make(map[string]*pfcec.PrivateKey),
...
var (
	...
	secSrc                    *secSource
	addr1, addr2, addr3       pfcutil.Address
...
func testRescan(harness *neutrinoHarness, t *testing.T) {
	...
	privKey1, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
	...
	privKey2, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		t.Fatalf("Couldn't generate private key: %s", err)
	}
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target pfcutil.Amount) (
		total pfcutil.Amount, inputs []*wire.TxIn,
		...
		return func(target pfcutil.Amount) (total pfcutil.Amount,
			inputs []*wire.TxIn, inputValues []pfcutil.Amount,
			...
			inputValues = []pfcutil.Amount{
				pfcutil.Amount(tx.TxOut[ourIndex].Value)}
	...
	privKey3, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		t.Fatalf("Couldn't generate private key: %s", err)
	}
...
func startRescan(t *testing.T, svc *neutrino.ChainService, addr pfcutil.Address,
	...
	rescan := neutrino.NewRescan(
		...
		neutrino.NotificationHandlers(
			rpcclient.NotificationHandlers{
				...
				OnRecvTx: func(tx *pfcutil.Tx,
					details *pfcjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
							details.Hash)
				...
				OnRedeemingTx: func(tx *pfcutil.Tx,
					details *pfcjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
