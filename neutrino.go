...
var (
	...
	// DisableDNSSeed disables getting initial addresses for Picfightcoin nodes
	// from DNS.
...
// OnVerAck is invoked when a peer receives a verack picfightcoin message and is used
...
// OnReject is invoked when a peer receives a reject picfightcoin message and is
...
// picfightcoin network type specified by chainParams.  Use start to begin syncing
// with peers.
	...
	if !DisableDNSSeed {
		...
		connmgr.SeedFromDNS(&s.chainParams, RequiredServices,
			s.nameResolver, func(addrs []*wire.NetAddress) {
				...
				// Picfightcoind uses a lookup of the dns seeder
